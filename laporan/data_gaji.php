...
<head>
  ...
  <tbody>
    ...
    <tr>
      <td><h5>Perihal</h5></td>
      <td>:</td>
      <td><h5>Seluruh Data Gaji Pegawai</h5></td>
    </tr>
  </tbody>
</table>
  ...
  <div class="container">
  <h2 class="text-center">DAFTAR DATA GAJI PEGAWAI</h2>
  ...
  <table class="table table-bordered table-hover table-sm" style="margin: 0 auto">
      ...
      if (isset($_POST['cetak'])) {
        ...
        $result = mysqli_query($koneksi, "SELECT * FROM gaji INNER JOIN absensi ON gaji.id_absensi=absensi.id_absensi INNER JOIN t_pegawai ON absensi.id_pg=t_pegawai.id_pg WHERE gaji.bulan LIKE '%$bulan%' AND gaji.tahun LIKE '%$tahun%'");

      }else{
        $result = mysqli_query($koneksi, "SELECT * FROM gaji INNER JOIN absensi ON gaji.id_absensi=absensi.id_absensi INNER JOIN t_pegawai ON absensi.id_pg=t_pegawai.id_pg");
      }
