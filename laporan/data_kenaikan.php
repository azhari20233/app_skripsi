...
<head>
  ...
  <title>Data Kenaikan Jabatan Pegawai</title>
...
if (isset($_POST['cetak'])) {
  ...
  $result = mysqli_query($koneksi, "SELECT * FROM kenaikan_jabatan INNER JOIN t_pegawai ON kenaikan_jabatan.nip=t_pegawai.nip WHERE kenaikan_jabatan.tanggal LIKE '%$tahun-$bulan%' GROUP BY kenaikan_jabatan.id_kenaikan");  
}?>
  ...
  <tbody>
    ...
    <tr>
      <td><h5>Perihal</h5></td>
      <td>:</td>
      <td><h5>Seluruh Data Kenaikan Jabatan Pegawai</h5></td>
    </tr>
  </tbody>
</table>
  ...
  <div class="container">
  <h2 class="text-center">DAFTAR KENAIKAN JABATAN PEGAWAI</h2>
...
$no = 1;
      while($data = mysqli_fetch_array($result)) {?>
          <tr class="text-center">
            ...
            <td><?= $data['golongan_baru'] ?></td>
          </tr>
        <?php } ?>
        <tr>
          <td colspan="5" class="text-center font-weight-bold">Jumlah Kenaikan Jabatan Pegawai</td>
          <td colspan="5" class="text-center font-weight-bold"><?= mysqli_num_rows($result) ?></td>
        </tr>
      </tbody>
    </thead>
  </table>
