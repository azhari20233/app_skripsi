  ...
  <div class="row">
    <!-- Dark table start -->
      <div class="col-4 mt-5">
          <div class="card">
              <div class="card-body">
                <h4 class="mb-4">Cetak Data Gaji Pegawai <i class="ti ti-server"></i></h4>
              <?php $qrytahun = mysqli_query($koneksi, "SELECT * FROM gaji INNER JOIN absensi ON gaji.id_absensi=absensi.id_absensi INNER JOIN t_pegawai ON absensi.id_pg=t_pegawai.id_pg GROUP BY YEAR(tahun) ASC"); ?>
                  ...
                  <form action="../laporan/data_gaji.php" method="post" target="_blank"> 
                    <table>
                            ...
                            <label class="form-label">Berdasarkan Tahun</label>
                              <div class="" style="margin-bottom: 10px;">
                                <select style="color: black;" name="tahun-cetak" class="form-control">
                                  ...
                                  <option><?php $formatwaktu = $row["tahun"];echo date('Y',strtotime($formatwaktu)); ?></option>
                                  <?php } ?>
                                  <?php } ?>
                                </select>
                              </div>
                          ...
                          <a title="Cetak Semua Data" href="../laporan/data_gaji.php?id_gaji=<?php echo $row['id_gaji']; ?>" class="btn btn-dark" target="_blank"><i class="mdi ti-check-box"></i></a>
                        </div>
                </form>
              </div>
          </div>
